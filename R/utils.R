...
#' @noRd
make_params <- function(api_key=Sys.getenv("CMC_API_KEY"),...){
  params <- list(...)
  params["CMC_PRO_API_KEY"] = api_key
  params <- plyr::compact(params)
  if ("time_start" %in% names(params)){
    params["time_start"] <- sanitize_date(params[["time_start"]])
  }
  if ("time_end" %in% names(params)){
